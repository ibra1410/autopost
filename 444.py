...
# تعديل بيدوووو @K_B_I اخمط وانت فاشل يا فاشل 😂❗❗
...
async def newSuper(_: Client, callback: CallbackQuery):
    ...
    try:
        ask = await listener.listen(
            from_id=user_id, 
            chat_id=user_id,
            text="- ارسل روابط السوبر لإضافتها (افصل بين الروابط بمسافة أو سطر جديد).\n- لا تنضم قبل أن تقوم بالنشر لمرة واحدة على الأقل.\n- إذا كان السوبر خاصاً، أرسل الأيدي الخاص به أو غادر السوبر (من الحساب المضاف) ثم أرسل الرابط.\n\n- يمكنك ارسال /cancel لإلغاء العملية.",
            reply_markup=ForceReply(selective=True, placeholder="- Super group URLs: "),
            timeout=120
        )
    except exceptions.TimeOut:
        return await callback.message.reply("نفذ وقت استلام الروابط", reply_markup=reMarkup)
    
    if ask.text == "/cancel":
        return await ask.reply("- تم إلغاء العملية", reply_to_message_id=ask.id, reply_markup=reMarkup)
    
    links = ask.text.split()
    added_chats = []
    
    for link in links:
        try:
            chat = await app.get_chat(link if "+" in link else (link.split("/")[-1]))
        except BotMethodInvalid:
            chat = link
        except Exception as e:
            print(e)
        ...
        users[str(user_id)]["groups"].append(chat.id if not isinstance(chat, str) else int(chat))
        added_chats.append(chat.id if not isinstance(chat, str) else int(chat))
    
    if added_chats:
        write(users_db, users)
        await ask.reply(
            f"- تمت إضافة {len(added_chats)} سوبر إلى القائمة.", 
            reply_markup=Markup([
                [Button("- إضافة سوبر -", callback_data="newSuper"), Button("- الصفحة الرئيسية -", callback_data="toHome")]
            ]),
            reply_to_message_id=ask.id
        )
    else:
        await ask.reply(
            "- لم يتم إضافة أي سوبر، تحقق من الروابط وحاول مرة أخرى.", 
            reply_markup=reMarkup,
            reply_to_message_id=ask.id
        )
